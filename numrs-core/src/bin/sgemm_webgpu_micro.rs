...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: sgemm_webgpu_micro <dim> <reps>");
        std::process::exit(1);
    }
    let dim: usize = args[1].parse().expect("dim must be int");
    let reps: usize = args[2].parse().expect("reps must be int");

    // Force WebGPU backend for this microbenchmark
    env::set_var("NUMRS_BACKEND", "webgpu");

    println!("sgemm_webgpu_micro: dim={} reps={}", dim, reps);

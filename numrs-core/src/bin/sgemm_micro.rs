...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: sgemm_micro <dim> <reps>");
        std::process::exit(1);
    }
    let dim: usize = args[1].parse().expect("dim must be int");
    let reps: usize = args[2].parse().expect("reps must be int");

    // Ensure BLAS backend if available
    env::set_var("NUMRS_BACKEND", "blas");

    println!("sgemm_micro: dim={} reps={}", dim, reps);

    let a = make_matrix(dim);
    let b = make_matrix(dim);

    // warmup
    let _ = numrs::ops::matmul(&a, &b).expect("matmul");

    let mut times = Vec::with_capacity(reps);
    for _ in 0..reps {
        let start = Instant::now();
        let out = numrs::ops::matmul(&a, &b).expect("matmul");
        let dur = start.elapsed();
        // touch result so compiler doesn't optimize away

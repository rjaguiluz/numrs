...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: sgemm_generic_micro <dim> <reps>");
        std::process::exit(1);
    }
    let dim: usize = args[1].parse().expect("dim must be int");
    let reps: usize = args[2].parse().expect("reps must be int");

    println!("sgemm_generic_micro: dim={} reps={}", dim, reps);

    let a = make_matrix(dim);
    let b = make_matrix(dim);

    // warmup (dispatch via numrs::ops::matmul — respects NUMRS_BACKEND)
    let _ = numrs::ops::matmul(&a, &b).expect("matmul warmup failed");
